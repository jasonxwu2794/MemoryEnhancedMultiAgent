...
# Build messaging config before heredoc
MESSAGING_CONFIG=""
case "$MESSAGING" in
    telegram)
        TELEGRAM_TOKEN="$(state_get 'telegram_token')"
        MESSAGING_CONFIG="  telegram_token: $TELEGRAM_TOKEN"
        ;;
    discord)
        DISCORD_TOKEN="$(state_get 'discord_token')"
        DISCORD_GUILD="$(state_get 'discord_guild_id')"
        MESSAGING_CONFIG="  discord_token: $DISCORD_TOKEN
  discord_guild: $DISCORD_GUILD"
        ;;
    signal)
        SIGNAL_PHONE="$(state_get 'signal_phone')"
        MESSAGING_CONFIG="  signal_phone: $SIGNAL_PHONE"
        ;;
esac

cat > "$PROJECT_DIR/openclaw.yaml" << EOF
# OpenClaw Gateway Configuration
# Generated by setup wizard — $(date -u +%Y-%m-%dT%H:%M:%SZ)
...
if has_cmd systemctl && systemctl is-enabled --quiet openclaw.service 2>/dev/null; then
    ...
    log_warn "OpenClaw not found — please start it manually"
fi
